		...
		const char *op_type_str)
{
	...
	snprintf(pool_name, sizeof(pool_name), "%s_pool_%u", op_type_str,
			dev_id);
	printf("DATA room size is: %d\n", RTE_MAX(max_seg_sz + RTE_PKTMBUF_HEADROOM
					+ FILLER_HEADROOM,
			(unsigned int)RTE_MBUF_DEFAULT_BUF_SIZE));
	printf("pool name is: %s\n", pool_name);
		...
		enum rte_bbdev_op_type org_op_type, uint16_t num_ops)
{
	...
	printf("number of queues for ops pool size are: %d\n", ad->nb_queues);
	printf("number of ops are: %d\n", num_ops);
	printf("rte lcore count are: %d\n", rte_lcore_count());
	...
	printf("pool name is: %s\n", pool_name);
	printf("op type is: %d\n", op_type);
	printf("ops pool size is:  %d\n", ops_pool_size);
	mp = rte_bbdev_op_pool_create(pool_name, op_type,
			ops_pool_size, OPS_CACHE_SIZE, socket_id);
	// printf("ops pool size is: %d\n", ops_pool_size);
	printf("INFO: Creating %u items ops pool for dev %u on socket %u.\n",
			ops_pool_size,
			ad->dev_id,
			socket_id);
	...
	/* Inputs */
	printf("nb_segments is: %d\n", in->nb_segments);
	if (in->nb_segments > 0) {
		mbuf_pool_size = optimal_mempool_size(ops_pool_size *
				in->nb_segments);
		printf("mbuf pool size is: %d\n", mbuf_pool_size);
	...
	if (hard_out->nb_segments > 0) {
		printf("hard output is being initialized!!!!!!!!!!!!!!\n");
		mbuf_pool_size = optimal_mempool_size(ops_pool_size *
	...
	if (soft_out->nb_segments > 0) {
		printf("soft output is being initialized!!!!!!!!!!!!!!\n");
		mbuf_pool_size = optimal_mempool_size(ops_pool_size *
 ...
 */
	...
	if ((get_init_device() == true) &&
		...
		printf("reset to default is being called !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
...
{
	printf("Populating active devices...\n");
	int ret;
	uint8_t dev_id;
	...
	RTE_BBDEV_FOREACH(dev_id) {
		...
		printf("current num of queues is: %d\n", info.num_queues);
		nb_devs_added++;
	}
	printf("Populated %u active devices\n", nb_devs_added);
...
ut_setup(void)
{
	...
	for (i = 0; i < nb_active_devs; i++) {
		...
		printf("Device %u (%s) setup\n", dev_id,
				active_devs[i].driver_name);
		/* reset bbdev stats */
	...
	for (i = 0; i < n; ++i) {
		...
		if ((op_type == DATA_INPUT) || (op_type == DATA_HARQ_INPUT)) {
			if ((op_type == DATA_INPUT) && large_input) {
				printf("large input in init_op_data_objs\n");
		...
		const uint16_t n, struct test_op_params *op_params)
{
	...
	printf("add filler being called\n");
	if (input_ops == NULL)
		return;
		...
		struct rte_bbdev_dec_op *ref_op)
{
	...
	printf("snr in generate llr input is %f\n", get_snr());

	for (i = 0; i < n; ++i) {
		// TODO: Changed here to give same input
		m = inputs[0].data;
		...
		struct rte_bbdev_dec_op *ref_op)
{
	...
	for (i = 0; i < n; ++i) {
		...
		// intentionally set it to a wrong value to check BLER
		// if (i == 1){
		// 	ops[i]->ldpc_dec.q_m = 2;
		// }
		// else {
		...
		if (hard_outputs != NULL)
			// if (i == 1){
			// 	printf("messing at i == 1 \n");
			// 	ops[i]->ldpc_dec.hard_output =
			// 		hard_outputs[start_idx + 1];
			// } else{
			ops[i]->ldpc_dec.hard_output =
		...
		if (inputs != NULL){
			...
			ops[i]->ldpc_dec.input =
					inputs[start_idx + i];
					// }
			// if (inputs[start_idx + i] == NULL){}
			// printf("start_idx + i is: %d \n", (start_idx+i));
		}
		if (soft_outputs != NULL)
...
#define MAX_SEGMENTS 256  // Define a reasonable limit for buffer size
		...
		struct op_data_entries *orig_op)
{
	...
	uintptr_t addresses[MAX_SEGMENTS];  // Buffer to store addresses
	...
	for (i = 0; i < nb_dst_segments; ++i) {
		addresses[i] = (uintptr_t)m;  // Store address of each mbuf
...
// If you wish to check data manually
// Ensure the length is a multiple of 4 for 32-bit output, truncate otherwise
    //    uint16_t num_words = data_len / 4;

    //    printf("output%d =\n", i);
    //    for (uint16_t j = 0; j < num_words; j++) {
    //        // Read 4 bytes at a time and print them as a 32-bit hex value
    //        uint32_t word = ((uint32_t)data[j*4 + 3] << 24) |
    //                        ((uint32_t)data[j*4 + 2] << 16) |
    //                        ((uint32_t)data[j*4 + 1] << 8)  |
    //                        ((uint32_t)data[j*4]);

    //        // Print the word in the required format
    //        printf("0x%08X", word);

    //        // Add comma and formatting for 8 words per line
    //        if (j % 8 == 7) {
    //            printf(",\n");
    //        } else {
    //            printf(", ");
    ...
    //    // If there's a remainder of bytes (i.e., if the data_len is not a multiple of 4)
    //    if (data_len % 4 != 0) {
    //        printf("\n// Remaining bytes that don't form a full 32-bit word:");
    //        for (uint16_t j = num_words * 4; j < data_len; j++) {
...
/* Check Number of code blocks errors */
static int
new_validate_ldpc_bler(struct rte_bbdev_dec_op **ops, const uint16_t n)
{
	unsigned int i;
	struct op_data_entries *hard_data_orig =
			&test_vector.entries[DATA_HARD_OUTPUT];
...
calc_ldpc_dec_TB_size(struct rte_bbdev_dec_op *op)
{
	...
	if (op->ldpc_dec.code_block_mode == RTE_BBDEV_CODE_BLOCK) {
		printf("in code block mode ! ! ! !\n");
		printf("sys_cols is: %d\n", sys_cols);
		printf("zc is: %d\n", op->ldpc_dec.z_c);
		printf("n_filler is: %d\n", op->ldpc_dec.n_filler);
		...
		uint16_t burst_sz, uint16_t num_to_process, uint16_t num_lcores)
{
	...
	printf("rte_bbdev_op_alloc here !!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		...
		struct test_op_params *op_params)
{
	...
	// printf("[2] added device name is: %s\n", info.dev_name);
	// printf("[2] added device socket id is: %d\n", info.socket_id);
	// printf("[2] added device driver name is: %s\n", info.drv.driver_name);
	// printf("[2] number of queues is: %d\n", info.num_queues);
	...
	printf("rte_bbdev_op_cout is: %d\n", RTE_BBDEV_OP_TYPE_COUNT);
	for (i = 0; i < RTE_BBDEV_OP_TYPE_COUNT; i++) {
		// printf("cap type outside test vector is %d\n", cap->type);
		if (cap->type == test_vector.op_type) {
			capabilities = cap;
			printf("cap type being tested is:  %d\n", cap->type);
			break;
		}
		cap++;
	}
...
static int
new_bler_pmd_lcore_ldpc_dec(void *arg)
{
	printf("in bler pmd lcore ldpc dec mode!!!!\n");
	struct thread_params *tp = arg;
	uint16_t enq, deq;
	uint64_t total_time = 0, start_time;
	const uint16_t queue_id = tp->queue_id;
	printf("queue_id in new bler pmd lcore ldpc dec is %d\n", queue_id);
	const uint16_t burst_sz = tp->op_params->burst_sz;
	printf("burst_sz in new bler pmd lcore ldpc dec is %d\n", burst_sz);
	const uint16_t num_ops = tp->op_params->num_to_process;
	printf("num_ops in new bler pmd lcore ldpc dec is %d\n", num_ops);
	// struct rte_bbdev_dec_op *ops_enq[num_ops];
	// struct rte_bbdev_dec_op *ops_deq[num_ops];

	// provide seperate input to ops_enq and ops_deq
	struct rte_bbdev_dec_op *ops_enq[num_ops];
	struct rte_bbdev_dec_op *ops_deq[num_ops];
	printf("new_bler: after init ops enq and deq\n");

	struct rte_bbdev_dec_op *ref_op = tp->op_params->ref_dec_op;
	printf("new_bler: after ref_op\n");	
	...
	printf("new_bler: before get dev info\n");
	rte_bbdev_info_get(tp->dev_id, &info);
	printf("new_bler: after get dev info\n");

	TEST_ASSERT_SUCCESS((num_ops > info.drv.queue_size_lim),
			"NUM_OPS cannot exceed %u for this device",
			info.drv.queue_size_lim);
	
	printf("new_bler: buffers\n");
	bufs = &tp->op_params->q_bufs[GET_SOCKET(info.socket_id)][queue_id];
	...
	if (test_vector.op_type != RTE_BBDEV_OP_NONE)
		copy_reference_ldpc_dec_op(ops_enq, num_ops, 0, 
				bufs->hard_outputs, bufs->inputs, bufs->soft_outputs,
				bufs->harq_inputs, bufs->harq_outputs, ref_op);
	generate_llr_input(num_ops, bufs->inputs, ref_op);
	...
	printf("loopback is %d\n", loopback);
	printf("hc_out is %d\n", hc_out);

// from here, do two enquques. 
	...
	for (i = 0; i < 1; ++i) { /* Could add more iterations */
		...
		for (enq = 0, deq = 0; enq < num_ops;) {
			...
			// if (unlikely(2 - enq < num_to_enq))
			// 	num_to_enq = 2 - enq;
			// printf("enqueue number is ");
			// printf("%d\n", enq);
			enq += rte_bbdev_enqueue_ldpc_dec_ops(tp->dev_id,
					queue_id, &ops_enq[enq], num_to_enq);
		...
		// enq = 0; deq = 0;
		// num_to_enq = burst_sz;
		// enq += rte_bbdev_enqueue_ldpc_dec_ops(tp->dev_id, queue_id, &ops_enq[0], num_to_enq);


		// printf("enqueue number is ");
		// printf("%d\n", enq);
		// printf("dequeue number is ");
		// printf("%d\n", deq);
		/* dequeue the remaining */
		...
		while (deq < enq) {
			// printf("dequeue before in while loop is: ");
			// printf("%d\n", deq);
			deq += rte_bbdev_dequeue_ldpc_dec_ops(tp->dev_id,
					queue_id, &ops_deq[deq], enq - deq);
			// printf("dequeue after in while loop is: ");
			// printf("%d\n", deq);
		}
		// printf("enqueue number is ");
		// printf("%d\n", enq);
	...
	printf("before getting bler\n");
	tp->bler = (double) validate_ldpc_bler(ops_deq, num_ops) / num_ops;
	// tp->bler = (double) validate_ldpc_bler(ops_deq, 1) / 1;
	printf("after getting bler\n");
	...
	printf("bler tb_len_bits is: %f\n", tb_len_bits);
	printf("total time is: %lu\n", total_time);
	printf("time per u-second is: %f\n", ((total_time / (double)rte_get_tsc_hz()) * 1000000));

	tp->ops_per_sec = ((double)num_ops * 1) /
			((double)total_time / (double)rte_get_tsc_hz());
	printf("num_ops here is: %d\n", num_ops);
	tp->mbps = (((double)(num_ops * 1 * tb_len_bits)) /
			1000000.0) / ((double)total_time /
			(double)rte_get_tsc_hz());
	
	// printf("time in us is: %f\n", ((((double)(num_ops * 1 * tb_len_bits))) / tp->mbps / (double)rte_get_tsc_hz()) );
...
bler_pmd_lcore_ldpc_dec(void *arg)
{
	...
	struct thread_params *tp = arg;
	uint16_t enq, deq;
	uint64_t total_time = 0, start_time, end_time;
	float time_array[iter_max];
	const uint16_t queue_id = tp->queue_id;
	printf("queue_id in bler pmd lcore ldpc dec is %d\n", queue_id);
	const uint16_t burst_sz = tp->op_params->burst_sz;
	printf("burst_sz in bler pmd lcore ldpc dec is %d\n", burst_sz);
	const uint16_t num_ops = tp->op_params->num_to_process;
	printf("num_ops in bler pmd lcore ldpc dec is %d\n", num_ops);
	// struct rte_bbdev_dec_op *ops_enq[num_ops];
	// struct rte_bbdev_dec_op *ops_deq[num_ops];
...
// from here, do two enquques. 
	...
	for (i = 0; i < iter_max; ++i) { /* Could add more iterations */
		// printf("init enq\n");
		...
		// printf("dequeue number is ");
		// printf("%d\n", deq);
		end_time = rte_rdtsc_precise();
		total_time += end_time - start_time;
		time_array[i] = ((end_time - start_time) / (double)rte_get_tsc_hz()) * 1000000;
	}
...
throughput_pmd_lcore_ldpc_dec(void *arg)
{
	...
	double tb_len_bits = calc_ldpc_dec_TB_size(ref_op);
	printf("ldpc tb_len_bits is: %f\n", tb_len_bits);
	printf("total time is: %lu\n", total_time);
	printf("time per u-second is: %f\n", ((total_time / (double)rte_get_tsc_hz()) * 1000000 / TEST_REPETITIONS));
	tp->ops_per_sec = ((double)num_ops * TEST_REPETITIONS) /
			((double)total_time / (double)rte_get_tsc_hz());
	tp->mbps = (((double)(num_ops * TEST_REPETITIONS * tb_len_bits)) /
			1000000.0) / ((double)total_time /
			(double)rte_get_tsc_hz());
	printf("mbps is: %f\n", tp->mbps);
	// printf("time in us is: %f\n", ((((double)(num_ops * TEST_REPETITIONS * tb_len_bits))) / tp->mbps / (double)rte_get_tsc_hz()) );
		...
		uint64_t *total_time, uint64_t *min_time, uint64_t *max_time)
{
	...
	printf("in latency_test_ldpc_enc\n");
		...
		struct test_op_params *op_params, bool latency_flag)
{
    ...
    printf("num_dev: %u\n",  num_dev);
	printf("is valid? %u\n", rte_bbdev_is_valid(temp_dev_id));
	...
	printf("+ ------------------------------------------------------- +\n");
	if (latency_flag)
		printf("== test: latency 1 1 1 \ndev:");
	else
		printf("== test: validation 1 1 1\ndev:");
	printf("%s, burst size: %u, num ops: %u, op type: %s\n",
			info.dev_name, burst_sz, num_to_process, op_type_str);

	printf("device id: %" PRIu8 "\n", ad->dev_id);
...
static int
ldpc_enc_dec_test(struct active_device *ad,
		struct test_op_params *op_params)
{
    ...
    uint16_t num_dev = rte_bbdev_count();
    printf("num_dev: %u\n",  num_dev);
	printf("ldpc enc dec is valid? %u\n", rte_bbdev_is_valid(temp_dev_id));
	...
	printf("before latency test enc\n");
	iter = latency_test_ldpc_enc(op_params->mp, bufs,
				op_params->ref_enc_op, ad->dev_id, queue_id,
				num_to_process, burst_sz, &total_time,
				&min_time, &max_time);
	...
	// 		&& !check_bit(test_vector.ldpc_dec.op_flags,
	// 		RTE_BBDEV_LDPC_LLR_COMPRESSION))
	bler_function = new_bler_pmd_lcore_ldpc_dec;
	// else
	// 	return TEST_SKIPPED;
...
static int
ldpc_enc_dec_tc(void)
{
	return run_test_case(ldpc_enc_dec_test);
}
...
static struct unit_test_suite bbdev_all_testsuite = {
	.suite_name = "BBdev all Tests",
	.setup = testsuite_setup,
	.teardown = testsuite_teardown,
	.unit_test_cases = {
		TEST_CASE_ST(ut_setup, ut_teardown, ldpc_enc_dec_tc),
...
REGISTER_TEST_COMMAND(all, bbdev_all_testsuite);
