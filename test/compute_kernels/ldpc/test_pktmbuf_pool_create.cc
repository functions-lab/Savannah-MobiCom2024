...
static int
init_op_data_objs_from_table(struct rte_bbdev_op_data *bufs,
...
static int
init_op_output_objs_from_table(struct rte_bbdev_op_data *bufs,
...
int main(int argc, char* argv[]) {
  ...
  t_params = static_cast<struct thread_params*>(rte_zmalloc(NULL, num_lcores * sizeof(struct thread_params), RTE_CACHE_LINE_SIZE));
  ...
  /* Configure all queues belonging to this bbdev device */
    ret = rte_bbdev_queue_configure(dev_id, q_id, &qconf);
		if (ret != 0) {
			printf(
					"Allocated all queues (id=%u) at prio%u on dev%u\n",
					q_id, qconf.priority, dev_id);
			qconf.priority++;
			ret = rte_bbdev_queue_configure(dev_id, q_id,
					&qconf);
		}
    if (ret != 0) {
			printf("All queues on dev %u allocated: %u\n",
  ...
  ops_mp = rte_bbdev_op_pool_create("RTE_BBDEV_OP_LDPC_DEC_poo", RTE_BBDEV_OP_LDPC_DEC,
      2047, OPS_CACHE_SIZE, rte_socket_id());
  ...
  rte_mempool * mbuf_pool = rte_pktmbuf_pool_create("bbdev_mbuf_pool", NB_MBUF, 256, 0,
...
//   const LDPCconfig& ldpc_config = cfg->LdpcConfig(dir);
  ...
  std::printf("pktmbuf alloc takes %.2f us \n", GetTime::CyclesToUs(end_pktmbuf_alloc - start_pktmbuf_alloc, freq_ghz));
  std::printf("Freeing pktmbuf takes %.2f us \n", GetTime::CyclesToUs(end_pktmbuf_free - start_pktmbuf_free, freq_ghz));
  std::printf("Total time takes %.2f us \n\n", GetTime::CyclesToUs((end_pktmbuf_alloc - start_pktmbuf_alloc) + (end_pktmbuf_free - start_pktmbuf_free), freq_ghz));
...
//   std::printf("ref_op_time %.2f us \n", GetTime::CyclesToUs(start_ref_op_create - end_ref_op_create, freq_ghz));
  ...
  size_t start_ref_op_create = GetTime::WorkerRdtsc();
    ...
    if (!check_bit(ref_dec_op[0]->ldpc_dec.op_flags, RTE_BBDEV_LDPC_ITERATION_STOP_ENABLE)){
